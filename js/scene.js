...
function build() {
  ...
  for (let r = 0; r < 5; r++) {
    ...
    branch(new V(0, 0.4, 0), new V(Math.cos(a), -0.06, Math.sin(a)).normalize(), 2.2, 0.2, 2, true); // surface roots: the camera sits near ground level
  }
  ...
  const pointsMat = (rise, colA, colB) => new THREE.ShaderMaterial({
    transparent: true, depthWrite: false, blending: THREE.NormalBlending, fog: false, // light background: additive glow would wash out
    uniforms: {
    ...
    fragmentShader: /* glsl */`
      ...
      void main() {
        ...
        vec3 c = mix(uA, uB, step(.95, vMix));
        gl_FragColor = vec4(c, smoothstep(0., .6, a) * vAlpha * mix(.85, .35, uRise));
      }`,
  });
  ...
  function points(count, place, rise, colA, colB, sizeMin, sizeMax) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      rand[i] = rnd();
      size[i] = sizeMin + rnd() * (sizeMax - sizeMin) + (rand[i] > 0.95 ? sizeMax * .5 : 0); // fruit = a little bigger
    }
